...
class PositionalEncoding2D(nn.Module):
    ...
    def __init__(self, d_model: int, max_h: int = 19, max_w: int = 11, persistent: bool = False) -> None:
        super().__init__()
...
class PositionalEncoding(torch.nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 20, persistent: bool = False) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
        # x.shape = (S, B, d_model)
        # [1,2,2,3]
        pe_indexed = torch.index_select(self.pe, 0, positions)
        assert x.shape[2] == pe_indexed.shape[2]  # type: ignore
        x = x + pe_indexed  # type: ignore
